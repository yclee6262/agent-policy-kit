...
function walkFiles(root, limit = 600) {
  const output = [];
...
export function collectInventory(root) {
  ...
  return {
    ...
    inventory_truncated: files.length >= 600,
...
function deterministicRules(inventory) {
  const rules = [];
  const scripts = inventory.package ? inventory.package.scripts : {};
  const checks = [
    ["test", "REPO-TEST-001", "執行 repository 測試命令"],
    ["lint", "REPO-QUALITY-001", "執行 repository lint 命令"],
    ["typecheck", "REPO-TYPE-001", "執行 repository typecheck 命令"],
    ["build", "REPO-BUILD-001", "執行 repository build 命令"],
  ];
  for (const [script, id, title] of checks) {
    if (!scripts || typeof scripts[script] !== "string") continue;
    const command = `npm run ${script}`;
    rules.push(ruleBlock(id, title, {
      severity: script === "test" ? "MUST" : "SHOULD",
      applies: "可能影響此命令結果的變更",
      requirement: `宣告相關工作完成前，必須執行 ${markdownCode(command)}。`,
      evidence: `${markdownCode("package.json")} 將 ${markdownCode(script)} 定義為 ${markdownCode(scripts[script])}；初始化期間未執行此命令。`,
      verification: `${markdownCode(command)} 必須以狀態碼 0 結束。OWNER CONFIRMATION REQUIRED。`,
      recovery: "修復失敗，或說明原因並將此命令標記為未驗證。",
    }));
  }
